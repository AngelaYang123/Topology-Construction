...
void get_graph(int constraint){
    ...
    filename = to_string(constraint) + "_AutoTag.txt";
...
void process(int constraint, int seed, int density, int computation){
    int i, j;
    int left, right;
...
int main(int argc, char *argv[]){
    ...
    for(dp=2; dp<=node_num; dp++){
        initial();
        process(dp, seed, density, computation);
        end();   
    }
    ...
    flag = 1;
    initial();
    process(constraint, seed, density, computation);
    end(); 
